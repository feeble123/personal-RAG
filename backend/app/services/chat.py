...
5. 公式用 LaTeX 排版：行内公式用 `$...$`，独立公式用 `$$...$$`（如 `$$v = C\sqrt{Ri}$$`），确保前端数学排版渲染。**严禁**使用 `\(...\)` 或 `\[...\]` 这种定界符，只能使用 `$...$` 和 `$$...$$`。
...
9. **引用编号纪律**：引用编号 [n] 必须精确对应当前【参考资料】中的条目（[1] 即第 1 条资料），只能标注当前参考资料中确实存在的编号。若某个内容或结论的依据来自【对话历史】或你的通用知识，而非当前【参考资料】，**不得**标注引用编号，应如实说明该内容未在当前参考资料中找到依据。**严禁**沿用、跳过或臆造历史对话中出现过的编号——每一轮问答的【参考资料】编号彼此独立，绝不与历史编号混淆。若【参考资料】共 N 条，**你只能使用 [1] 到 [N] 的编号，严禁出现 [N+1] 及更大的编号**（那是不存在的资料）。列举多个要点时，**每个要点都必须标注对应引用编号**，不得整段不标引用。"""
