...
- 查询向量 LRU 缓存（降延迟）
- 文档向量按 content_hash 走 DB 缓存（省 API 调用）
"""
...
def _cached_query_vector(model: str, text: str) -> tuple[float, ...]:
...
async def embed_query(text: str) -> list[float]:
    """异步查询向量：LRU 缓存 + 线程池。"""
    vector = await asyncio.to_thread(_cached_query_vector, settings.embedding_model, text)
    return list(vector)
...
async def load_cache_vectors(db: AsyncSession, hashes: list[str]) -> dict[str, list[float]]:
    ...
    rows = await db.scalars(
        select(EmbeddingCache).where(
            ...
            EmbeddingCache.model_version == settings.embedding_model,
        )
    )
    import json
...
async def store_cache_vectors(db: AsyncSession, vectors: dict[str, list[float]]) -> None:
    """写入 embedding_cache 表（重复 key 跳过）。"""
    if not settings.embedding_cache_enabled or not vectors:
        return
    import json

    existing = await db.scalars(
        select(EmbeddingCache.content_hash).where(EmbeddingCache.content_hash.in_(list(vectors)))
    )
